...
const Explore = () => {
  ...
  // Modified function to check if user can view visit details
  const goToDetails = async (visit, e) => {
    if (e.target.tagName === "BUTTON" || e.target.closest("button")) {
      e.preventDefault();
      return;
    }

    // Check if visit is locked/completed and if user hasn't joined
    ...
    const isOrganizer = visit.user._id === user._id;

    // Prevent access if visit is locked/completed and user hasn't joined and isn't the organizer
    if (isLockedOrCompleted && !hasJoined && !isOrganizer) {
      toast.info(
        "You cannot view details of this visit as it is locked or completed and you are not a participant."
      );
      return;
    }

    try {
      await dispatch(fetchVisitDetails(visit._id));
  ...
  // Function to determine if a visit card should be clickable
  const isVisitClickable = (visit) => {
    const isLockedOrCompleted =
      visit.status === "locked" || visit.status === "completed";
    const hasJoined = visit.joinRequests.some(
      (req) => req.user === user._id || req.user?._id === user._id
    ...
    return !isLockedOrCompleted || hasJoined || isOrganizer;
  };
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100">
        <div className="max-w-md w-full p-8 bg-white rounded-2xl shadow-xl border border-purple-100 animate-fade-in-up">
          ...
          <p className="text-gray-600 mb-8 text-center">
            Join amazing visits with fellow explorers and create memorable
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-purple-50 to-indigo-100">
        ...
        <p className="text-purple-800 font-medium text-lg">
          Loading amazing visits for you...
        </p>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-purple-50 to-indigo-100">
        <div className="bg-white p-8 rounded-xl shadow-lg border-l-4 border-red-500 max-w-md w-full">
          <p className="font-bold text-red-700 text-xl mb-2">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          ...
          >
            ...
            <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-black bg-opacity-30">
              +
            </span>
          </Link>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-md mb-6 flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2 border-b-2 border-gray-200 rounded-lg focus:ring-purple-500 focus:border-purple-500"
          ...
          <>
                ...
                <div className="mb-8">
                  ...
                  <div
                    className={`bg-white rounded-xl shadow-lg overflow-hidden ${
                      isVisitClickable(filteredVisits[0])
                        ? "cursor-pointer"
                        : "cursor-not-allowed opacity-90"
                    } transform transition-all duration-500 border-2 border-purple-200 hover:border-purple-400 ${
                      animateCards
                    ...
                    <div className="relative h-36 bg-gradient-to-r from-indigo-600 via-purple-500 to-pink-500">
                      ...
                      {(filteredVisits[0].status === "locked" ||
                        ...
                        filteredVisits[0].user._id !== user._id && (
                          <div className="absolute inset-0 bg-purle bg-opacity-40 flex items-center justify-center">
                            <div className="bg-black bg-opacity-20 p-4 rounded-full backdrop-blur-sm">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 flex justify-between items-end p-4">
                        ...
                        <div className="flex items-center bg-black bg-opacity-30 backdrop-blur-sm rounded-lg px-3 py-1.5">
                          ...
                          <span className="text-sm text-white font-medium">
                            {filteredVisits[0].joinRequests.length}/
                            {filteredVisits[0].maxParticipants}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-6">
                      ...
                      {filteredVisits[0].user._id === user._id ? (
                        ...
                        <div className="w-full bg-gray-100 text-gray-700 py-2.5 rounded-lg text-center font-medium shadow-sm">
                          Cannot Join -{" "}
                          {filteredVisits[0].status.charAt(0).toUpperCase() +
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredVisits.map((visit, index) => {
                ...
                if (index === 0 && activeFilter === "all" && !searchTerm)
                  return null;
                ...
                const canViewDetails =
                  !isLockedOrCompleted || hasJoined || isOrganizer;
                ...
                return (
                  <div
                    key={visit._id}
                    className={`group bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-500 border border-purple-50 ${
                      canViewDetails
                        ? "cursor-pointer"
                        : "cursor-not-allowed opacity-90"
                    } transform ${
                      canViewDetails ? "hover:-translate-y-2" : ""
                    } ${
                      animateCards
                        ? "translate-y-0 opacity-100"
                        : "translate-y-4 opacity-0"
                    ...
                    onClick={(e) => canViewDetails && goToDetails(visit, e)}
                  >
                    <div className="relative h-28 bg-gradient-to-r from-indigo-600 to-purple-500 flex items-end">
                      <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black opacity-30"></div>
                      {/* Show lock overlay for locked/completed visits that user hasn't joined */}
                      {isLockedOrCompleted && !hasJoined && !isOrganizer && (
